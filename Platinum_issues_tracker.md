...
| API-005 | 🟢 | High | POS-API `/api/auth/login` granted demo session BEFORE validating credentials | RESOLVED 2026-04-21: Demo bypass now gated by `POS_AUTH_DEMO_MODE` env var **AND** `NODE_ENV !== 'production'`. Currently set to `true` in the **development** environment (POS-API workflow registered: `cd POS-API && PORT=3003 npx tsx index.ts`). When OFF: bad credentials return 401, missing password returns 400, only Azure-validated users get a session. When ON (dev only): instant demo session as before with background Azure upgrade. Verified both modes via curl. |
