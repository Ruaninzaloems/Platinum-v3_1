...
public class CriticalityGradeController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CriticalityGrade model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var dup = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_Criticality_Grade"" WHERE ""AssetCriticalityGradeDesc"" ILIKE @CriticalityGradeDesc", new { model.CriticalityGradeDesc }) > 0;
        if (dup) return Conflict(new { error = $"Criticality grade '{model.CriticalityGradeDesc}' already exists" });
        var id = await conn.QuerySingleAsync<int>(@"
            INSERT INTO ""Const_Asset_Criticality_Grade"" (""AssetCriticalityGradeDesc"", ""Enabled"", ""DateCaptured"", ""CapturerID"", ""Default"")
            VALUES (@CriticalityGradeDesc, @Enabled, NOW(), @CapturerID, @Default)
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CriticalityGrade model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var dup = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_Criticality_Grade"" WHERE ""AssetCriticalityGradeDesc"" ILIKE @CriticalityGradeDesc AND ""AssetCriticalityGradeID"" <> @id", new { model.CriticalityGradeDesc, id }) > 0;
        if (dup) return Conflict(new { error = $"Criticality grade '{model.CriticalityGradeDesc}' already exists" });
        var rows = await conn.ExecuteAsync(@"
            UPDATE ""Const_Asset_Criticality_Grade""
            SET ""AssetCriticalityGradeDesc"" = @CriticalityGradeDesc, ""Enabled"" = @Enabled, ""DateModified"" = NOW()
    ...
    public async Task<IActionResult> Export()
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var rows = await conn.QueryAsync<dynamic>(@"SELECT * FROM ""Const_Asset_Criticality_Grade"" ORDER BY ""AssetCriticalityGradeID""");
        using var workbook = new XLWorkbook();
        var ws = workbook.Worksheets.Add("Criticality Grades");
        ws.Cell(1, 1).Value = "ID";
        ws.Cell(1, 2).Value = "Criticality Grade Description";
        ws.Cell(1, 3).Value = "Consequence of Failure";
        ws.Cell(1, 4).Value = "Qualitative Description";
        ws.Cell(1, 5).Value = "Enabled";
        ws.Row(1).Style.Font.Bold = true;
        int r = 2;
        foreach (var row in rows)
        {
            ws.Cell(r, 1).Value = (int)row.AssetCriticalityGradeID;
            ws.Cell(r, 2).Value = (string?)row.AssetCriticalityGradeDesc ?? "";
            ws.Cell(r, 3).Value = (string?)row.ConsequenceOfFailure ?? "";
            ws.Cell(r, 4).Value = (string?)row.QualitiveDesc ?? "";
        ...
        stream.Position = 0;
        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CriticalityGrades_Export.xlsx");
    }
    ...
    public async Task<IActionResult> Import(IFormFile file)
    {
        ...
        for (int r = 2; r <= ws.LastRowUsed()?.RowNumber(); r++)
        {
            ...
            if (!seen.Add(val))
            {
                errors.Add(new ImportError { Row = r, Column = "Criticality Grade", Value = val, Message = $"Duplicate: 'Criticality Grade' value '{val}' in file" });
                continue;
            }
            rows.Add(val);
            rowNums[val] = r;
        ...
        {
            var exists = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_Criticality_Grade"" WHERE ""AssetCriticalityGradeDesc"" ILIKE @val", new { val }, txn) > 0;
            if (exists) { dbErrors.Add(new ImportError { Row = rowNums.TryGetValue(val, out var rn) ? rn : 0, Column = "Criticality Grade", Value = val, Message = $"Duplicate: '{val}' already exists in the database" }); continue; }
