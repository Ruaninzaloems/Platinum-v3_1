...
[ApiController]
[Route("api/disposal-methods")]
public class DisposalMethodController : ControllerBase
{
    private readonly DbConnectionFactory _db;

    public DisposalMethodController(DbConnectionFactory db) => _db = db;
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        var items = await conn.QueryAsync<DisposalMethod>(@"SELECT * FROM ""Const_AssetDisposalMethod"" ORDER BY ""AssetDisposalMethod_ID""");
        return Ok(items);
    }
    ...
    public async Task<IActionResult> GetById(int id)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var item = await conn.QueryFirstOrDefaultAsync<DisposalMethod>(@"SELECT * FROM ""Const_AssetDisposalMethod"" WHERE ""AssetDisposalMethod_ID"" = @id", new { id });
        return item is null ? NotFound(new { error = "Disposal method not found" }) : Ok(item);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] DisposalMethod model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] DisposalMethod model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
    ...
    public async Task<IActionResult> Delete(int id)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var rows = await conn.ExecuteAsync(@"DELETE FROM ""Const_AssetDisposalMethod"" WHERE ""AssetDisposalMethod_ID"" = @id", new { id });
        return rows == 0 ? NotFound(new { error = "Disposal method not found" }) : Ok(new { success = 1 });
    }
    ...
    public IActionResult GetTemplate()
    {
        ...
        var ws = workbook.Worksheets.Add("Disposal Methods");
        ws.Cell(1, 1).Value = "Disposal Method";
        ws.Cell(2, 1).Value = "Sale";
        ws.Row(1).Style.Font.Bold = true;
        using var stream = new MemoryStream();
        workbook.SaveAs(stream);
        stream.Position = 0;
        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DisposalMethods_Template.xlsx");
    }
