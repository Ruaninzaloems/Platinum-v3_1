...
public class AssetTypeController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        var items = await conn.QueryAsync<dynamic>(@"
            SELECT ""AssetType_ID"" AS ""assetTypeId"", ""AssetTypeDesc"" AS ""assetTypeDesc"", ""Enabled"" AS ""enabled""
            FROM ""Const_AssetType_Sys"" ORDER BY ""AssetType_ID""");
        return Ok(items);
    }
    ...
    public async Task<IActionResult> Import(IFormFile file)
    {
        ...
        foreach (var val in rows)
        {
            await conn.ExecuteAsync(@"
                INSERT INTO ""Const_AssetType_Sys"" (""AssetTypeDesc"", ""Enabled"", ""DateCaptured"", ""CapturerID"", ""Default"")
