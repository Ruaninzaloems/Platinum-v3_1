...
public class WipFundingTypeController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] WipFundingType model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var dup = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_WIPFundingType"" WHERE ""TypeDesc"" ILIKE @WIPFundingTypeDesc", new { model.WIPFundingTypeDesc }) > 0;
        if (dup) return Conflict(new { error = $"WIP funding type '{model.WIPFundingTypeDesc}' already exists" });
    ...
    public async Task<IActionResult> Update(int id, [FromBody] WipFundingType model)
    {
        await using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var dup = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_WIPFundingType"" WHERE ""TypeDesc"" ILIKE @WIPFundingTypeDesc AND ""FundingTypeID"" <> @id", new { model.WIPFundingTypeDesc, id }) > 0;
        if (dup) return Conflict(new { error = $"WIP funding type '{model.WIPFundingTypeDesc}' already exists" });
    ...
    public async Task<IActionResult> Import(IFormFile file)
    {
        ...
        for (int r = 2; r <= ws.LastRowUsed()?.RowNumber(); r++)
        {
            ...
            if (!seen.Add(val))
            {
                errors.Add(new ImportError { Row = r, Column = "Funding Type", Value = val, Message = $"Duplicate: 'Funding Type' value '{val}' in file" });
                continue;
            }
            rows.Add(val);
            rowNums[val] = r;
        ...
        {
            var exists = await conn.ExecuteScalarAsync<int>(@"SELECT COUNT(1) FROM ""Const_Asset_WIPFundingType"" WHERE ""TypeDesc"" ILIKE @val", new { val }, txn) > 0;
            if (exists) { dbErrors.Add(new ImportError { Row = rowNums.TryGetValue(val, out var rn) ? rn : 0, Column = "WIP Funding Type", Value = val, Message = $"Duplicate: '{val}' already exists in the database" }); continue; }
