...
public class DocumentsController : ControllerBase
{
    ...
    public DocumentsController(DbConnectionFactory db)
    {
        _db = db;
    ...
    [HttpPost("upload")]
    public async Task<IActionResult> Upload(IFormFile file, [FromForm] string? entity_type, [FromForm] string? entity_id, [FromForm] string? description)
    {
        ...
        var result = await conn.QueryFirstAsync<dynamic>(@"
            INSERT INTO ""Asset_Documents"" (""entity_type"", ""entity_id"", ""file_name"", ""file_path"", ""file_size"", ""mime_type"", ""uploaded_by"", ""description"")
            VALUES (@entity_type, @entity_id, @originalName, @fileName, @size, @mimeType, 1, @description) RETURNING *",
            new { entity_type, entity_id, originalName = file.FileName, fileName, size = file.Length, mimeType = file.ContentType, description });
