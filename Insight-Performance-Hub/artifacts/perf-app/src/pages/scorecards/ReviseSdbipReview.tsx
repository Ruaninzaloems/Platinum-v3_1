...
import {
  ...
  Loader2, ClipboardCheck, AlertTriangle,
} from "lucide-react";
...
export default function ReviseSdbipReview() {
  ...
  const scorecards = allScorecards?.filter(s =>
    (s.status === "Submitted" || s.status === "Reviewed") && s.approvedAt
  ) ?? [];
  ...
  const handleReviewKpi = async (kpiId: number, action: "review" | "return") => {
    ...
    try {
      await transitionKpi.mutateAsync({ id: kpiId, data: { action, comments: kpiComments[kpiId] || "" } });
  ...
  const handleReviewScorecard = async () => {
    ...
    setIsProcessing(true);
    try {
      await transitionScorecard.mutateAsync({ id: selectedScorecardId, data: { action: "review", comments: reviewComments } });
      toast({ title: "Revised SDBIP marked as Reviewed" });
      setSelectedScorecardId(undefined);
      setReviewComments("");
      refetchScorecards();
    } catch (e: any) {
      toast({ title: "Error", description: e?.response?.data?.error || "Failed to review", variant: "destructive" });
    } finally {
      setIsProcessing(false);
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-slate-800">Review Revised: {selectedScorecard.name}</h1>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card><CardContent className="pt-4"><div className="text-sm text-slate-500">Reviewed</div><div className="text-2xl font-bold text-amber-600">{reviewedKpis.length}</div></CardContent></Card>
          <Card><CardContent className="pt-4"><div className="text-sm text-slate-500">Pending Review</div><div className="text-2xl font-bold text-blue-600">{submittedKpis.length}</div></CardContent></Card>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Revised KPI Review</CardTitle>
            <CardDescription>Review each revised KPI. Check that revision reasons are adequate for any changed targets.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {kpis?.map(kpi => (
                <RevisionKpiReviewCard
                  key={kpi.id}
                  kpi={kpi}
        ...
        {selectedScorecard.status === "Submitted" && (
          <Card className="border-amber-200 bg-amber-50/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><ClipboardCheck className="w-5 h-5 text-amber-600" /> Revised Scorecard Review Decision</CardTitle>
              <CardDescription>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Textarea value={reviewComments} onChange={e => setReviewComments(e.target.value)} placeholder="Add review comments..." rows={3} />
              </div>
              <div className="flex gap-3">
                <Button onClick={handleReviewScorecard} disabled={!allKpisReviewed || isProcessing} className="bg-amber-600 hover:bg-amber-700">
                  {isProcessing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-2" />} Mark as Reviewed
                </Button>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Review Revised SDBIP</h1>
        <p className="text-slate-500 mt-1">Review mid-year revised scorecards submitted for approval. Verify that revision reasons are valid.</p>
      </div>
      <div className="flex items-center gap-4">
        <div className="w-64">
      ...
      {scorecards.length === 0 ? (
        <Card><CardContent className="py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-600">No Revised Scorecards Pending Review</h3>
          <p className="text-slate-400 mt-1">Submitted revised scorecards will appear here.</p>
        </CardContent></Card>
      ) : (
...
function RevisionKpiReviewCard({ kpi, comment, onCommentChange, onReview, onReturn, isProcessing, getUserName }: {
  kpi: any; comment: string; onCommentChange: (v: string) => void; onReview: () => void; onReturn: () => void; isProcessing: boolean; getUserName: (id: number | null) => string;
}) {
  ...
  return (
    <div className="border rounded-lg p-4 space-y-3">
      <div className="flex items-start justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={() => setExpanded(!expanded)}>{expanded ? "Collapse" : "Expand"}</Button>
      </div>
        ...
        <div className="space-y-3 pt-2 border-t">
          ...
          {kpi.status === "Submitted" && (
            <div className="space-y-2">
              <Textarea value={comment} onChange={e => onCommentChange(e.target.value)} placeholder="Review comments..." rows={2} className="text-sm" />
          ...
          {kpi.status === "Reviewed" && <div className="flex items-center gap-2 text-sm text-amber-600"><CheckCircle2 className="w-4 h-4" /> Reviewed</div>}
          {kpi.status === "Approved" && <div className="flex items-center gap-2 text-sm text-green-600"><CheckCircle2 className="w-4 h-4" /> Approved</div>}
        </div>
      )}
    </div>
  );
