...
export class PosBasketService {
  ...
  updateItemTender(id: string, tender: ItemTenderType, cashAmt?: number, cardAmt?: number, cardNum?: string, cardExp?: string): void {
    this.items.update((items) =>
      items.map((item) =>
        item.id === id
          ? { ...item, itemTenderType: tender, itemCashAmount: cashAmt, itemCardAmount: cardAmt, itemCardNumber: cardNum, itemCardExpiry: cardExp }
          : item
      )
    );
  }
