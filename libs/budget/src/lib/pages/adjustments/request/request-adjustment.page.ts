...
@Component({
  ...
  templateUrl: './request-adjustment.page.html',
  styleUrls: ['./request-adjustment.page.scss']
})
export class RequestAdjustmentPage implements OnInit {
  ...
  captureNewProject() {
    this.router.navigate(['/adjustments/capture']);
  }

  captureEditProject(p: ProjectItem) {
    this.router.navigate(['/adjustments/capture', p.id]);
  }
  ...
  loadProjects() {
    this.api.getProjects(this.filterDept || undefined, this.filterType || undefined).subscribe(resp => this.projects = resp.items);
  }
