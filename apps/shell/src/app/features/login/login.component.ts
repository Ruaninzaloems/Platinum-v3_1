...
@Component({
    ...
    <div class="login-page">
      ...
      <div class="login-right">
        <div class="login-card">
          ...
          <!-- ── Quick admin re-entry ─────────────────────────────────
               This system runs with login disabled (every visitor is an
               admin). After an explicit Sign Out this restores that session. -->
          <button type="button" class="admin-btn" (click)="continueAsAdmin()">
  ...
  styles: [`
    ...
    .admin-btn {
      width: 100%; height: 48px; border: 1px solid #cbd5e1; border-radius: 8px;
      background: #fff; color: #334155; font-size: .95rem; font-weight: 600;
      font-family: inherit; cursor: pointer;
      display: flex; align-items: center; justify-content: center; gap: .5rem;
      transition: background .15s, border-color .15s;
    }
    .admin-btn:hover { background: #f8fafc; border-color: #94a3b8; }
    .admin-btn .material-icons { font-size: 20px; color: #475569; }
...
export class LoginComponent implements OnInit {
  ...
  // ── Quick admin re-entry (login is disabled by design) ───────────────────
